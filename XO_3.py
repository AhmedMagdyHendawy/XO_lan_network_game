...
class Master(Tk):
    ...
    def actor_choice(self,choice):
        if (self.actor==""):
            self.actor=choice
            if not (self.opp):
                self.PC_socket.send((self.actor+",").encode('ascii'))
            ...
            if (self.Start):
                while True:
                    try:
                        ...
                        self.container.itemconfig(self.text, state='normal')
                        while True:
                            self.PC_socket.send(",".encode('ascii'))
                            if not(self.actor == ""):
                                while True:

                                    self.Opp_data =self.PC_socket.recv(BUFFER_SIZE)
                                    self.Opp_data=self.Opp_data.decode("ascii")
            ...
            if not (self.finish):
                if(self.opp):
                    ...
                    if(self.opp_turn):
                        ...
                        self.PC_socket.send(self.data_str.encode("ascii"))
                        self.result(i,j,self.opp)
                        self.opp_turn=True
                        if not(self.finish):
                            self.container.itemconfig(self.text, state='hidden')
